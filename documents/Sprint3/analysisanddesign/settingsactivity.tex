...
\begin{itemize}
	...
	As this User Interface component is useful to all applications, it should be implemented by the group responsible for the \giraf GUI library.\thilemann{Giraf components or is this fine?}
...
\paragraph{The challenge with Implementing Settings} using the above concepts is how to allow applications other than \launcher to access the settings User Interface, running in the context of \launcher as an activity.\thilemann{Maybe add that it is difficult to share between applications? dont think it is well introduced.}
...
While Android provides facilities for inter-application communication through its \textit{Broadcast} mechanism\cite{broadcastReceiver}, this requires all the involved applications to be running.\thilemann{Are we sure that the applications needs to be running??}
This is not an acceptable condition in this situation. 
\end{itemize}
 
\subsubsection{The Final Design}
\thilemann{Do we have to use \settingsactivity here already?}
...
With this design it is not possible to load settings from other applications as fragments from \launcher. Instead we start their application to access their settings.
However, this simplifies the complexity about saving the settings.
\thilemann[inline]{Rephrase below, badly formulated, too much use of we...}
...
By using fragments, it is also possible to add multiple fragments to a screen without switching activity.\thilemann{Hm, not by using fragments, but by using fragmentmanager? Or just rephrase.}
...
When the selected profile changes, settings activity should be reloaded to reflect the newly selected users settings.\thilemann{Hvem er det som ikke bruger ' korrekt. F.eks. user's settings = user has/is settings :/}
